    ...
    try:
        ...
        for vm in vms_usuarios:
            if vm.login not in logins_na_lista:
                # Criamos um objeto Pessoa para o VM que só existe como Usuário
                # Usamos um objeto que simula o modelo para passar pelo _build_response
                vm_virtual = Pessoa(
                    id=-(vm.id), 
                    nome=vm.nome,
                    loja_id=loja_id,
        ...
        for p in pessoas_ordenadas:
            try:
                ...
                # Ajuste manual para o VM virtual
                if p.id < 0:
                    item.cargo_nome = "Venerável Mestre"
                    item.id = p.id
                res.append(item)
            except Exception as e:
                print(f"Erro ao buildar resposta para {p.id}: {e}")
                continue
                
        return res
    except Exception as e:
        print(f"Erro crítico na listagem da loja {loja_id}: {e}")
        return []
...
def criar_pessoa(pessoa: PessoaCreate, db: Session = Depends(get_db)):
    ...
    nova = Pessoa(
        ...
        cargo_id=pessoa.cargo_id if pessoa.cargo_id != 0 else None,
        estado_id=estado.id,
        loja_id=pessoa.loja_id if pessoa.loja_id != 0 else None,
        login=pessoa.login,
        senha=pessoa.senha,
        ...
        indicador_id=pessoa.indicador_id if pessoa.indicador_id != 0 else None,
