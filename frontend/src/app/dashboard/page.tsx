...
export default function Dashboard() {
    ...
    const [user, setUser] = useState<{ estado: string; tipo: string; role?: string } | null>(null);
    ...
    // The navigateToCadastro function is now simplified to always go to the state map for master,
    // and directly to the state's page for state users.
    // The card's onClick will handle the specific navigation based on user type.
    const navigateToCadastro = () => {
        // This function is no longer directly used by the card,
        // as the card's onClick will be updated to handle the logic inline.
        // Keeping it here for now, but it will be effectively replaced by the card's new onClick.
        const userRole = user?.role || user?.tipo;
        if (userRole === 'master' || userRole === 'mestre') {
            router.push('/mapa'); // Master goes to map to select state
        } else if (user?.estado) {
            router.push(`/estado/${user.estado}`); // State user goes directly to their list
        }
    };
        ...
        <div className="min-h-screen bg-masonic-blue text-gray-100 font-serif overflow-hidden relative selection:bg-masonic-gold selection:text-masonic-blue">
            ...
            <main className="p-8 md:p-12 max-w-7xl mx-auto relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    {/* Card Admin (Only Master) */}
                    {(user.role === 'master' || user.tipo === 'master' || user.role === 'mestre') && (
                        <div
                    ...
                    {/* Card Trabalhos (Placeholder) */}
                    <div className="group relative bg-black/20 backdrop-blur-sm rounded-xl p-8 border border-white/5 opacity-50 cursor-not-allowed hover:border-white/10 transition-all">
                        <div className="w-14 h-14 bg-white/5 rounded-lg flex items-center justify-center mb-8 text-3xl border border-white/10">
                            🏗️
                        </div>

                        <h3 className="text-xl font-bold text-gray-400 mb-3 uppercase tracking-widest font-serif">
                            Trabalhos
                        </h3>
                        <p className="text-sm text-gray-600 leading-relaxed font-light">
                            Em breve: Registro detalhado de pranchas, arquitetura e controle de presenças em Loja.
                        </p>

                        <div className="mt-8 flex items-center text-gray-600 text-xs font-bold uppercase tracking-[0.2em]">
                            Em Breve
                        </div>
                    </div>

                    {/* Card Financeiro (Placeholder) */}
                    <div className="group relative bg-black/20 backdrop-blur-sm rounded-xl p-8 border border-white/5 opacity-50 cursor-not-allowed hover:border-white/10 transition-all">
                        <div className="w-14 h-14 bg-white/5 rounded-lg flex items-center justify-center mb-8 text-3xl border border-white/10">
                            💰
                        </div>
                        ...
                        <p className="text-sm text-gray-600 leading-relaxed font-light">
                            Em breve: Gestão completa de troncos de beneficência, capitação e balanços financeiros.
                        </p>
