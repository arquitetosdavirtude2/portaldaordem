...
export default function FormCadastro({ 
  ...
  useEffect(() => {
    ...
    // Carregar indicadores da loja
    if (lojaId) {
      fetch(`${apiUrl}/api/academia/indicadores/${lojaId}`)
        .then(r => r.ok ? r.json() : [])
        .then(setIndicadores)
        .catch(e => console.error("Erro ao buscar indicadores:", e));
    }
  }, [lojaId]);
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          cargo_id: tipoPessoa === 'candidato' ? 0 : (cargoId || 0),
          loja_id: (tipoPessoa === 'candidato' && !lojaId) ? 0 : (lojaId || acesso?.loja_id || 0),
          login: tipoPessoa === 'candidato' ? null : (login.trim() || null),
          senha: tipoPessoa === 'candidato' ? null : (senha.trim() || (isEditing ? null : "")),
          data_admissao: tipoPessoa === 'obreiro' ? (dataAdmissao || dataIniciacao || new Date().toISOString().split('T')[0]) : null,
          ativo: tipoPessoa === 'candidato' ? 1 : ativo,
          data_adormecimento: (tipoPessoa === 'candidato' || ativo === 1) ? null : dataAdormecimento,
          tipo_ingresso: tipoIngresso,
          indicador_id: indicadorId || 0,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
               ...
               <div>
                ...
                >
                  <option value="" className="bg-[#0a0a0a]">Nenhum (Indicação Direta)</option>
                  {indicadores.map(ind => (
