    ...
    if (carregando) {
        return (
            <div className="text-center py-20 text-yellow-500/50 flex flex-col items-center gap-4">
                ...
                <span className="text-[10px] uppercase font-medium tracking-[0.3em]">Cruzando dados dos irmãos...</span>
            </div>
        );
    }
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-wrap items-center gap-2 bg-black/20 p-2 rounded-xl border border-white/5">
                ...
                <div className="flex items-center gap-1 bg-black/40 p-1 rounded-lg border border-white/10 shadow-inner">
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-md text-[9px] font-medium uppercase tracking-wider transition-all flex items-center gap-1.5 ${visaoAtiva === 'ativos' ? 'bg-blue-500/20 text-blue-400' : 'text-gray-500 hover:text-gray-300'}`}
                    >
                        Ativos
                    </button>
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-md text-[9px] font-medium uppercase tracking-wider transition-all flex items-center gap-1.5 ${visaoAtiva === 'inadimplentes' ? 'bg-red-500/20 text-red-400' : 'text-gray-500 hover:text-gray-300'}`}
                    >
                        Inadimplentes
                    </button>
                </div>
                ...
                {/* Filtro de Categoria */}
                <div className="flex items-center gap-1 bg-black/40 p-1 rounded-lg border border-white/10 shadow-inner">
                    <span className="text-[8px] font-medium text-gray-500 uppercase px-1.5">Exibir:</span>
                    <select
                        value={categoriaFiltro}
                        onChange={(e) => setCategoriaFiltro(e.target.value as any)}
                        className="h-7 bg-[#0a0a0a] border border-white/10 rounded-md px-2 text-[9px] font-medium text-yellow-500 focus:outline-none transition-colors uppercase cursor-pointer"
                    >
                ...
                <button
                    ...
                    className={`h-9 px-3 rounded-lg text-[9px] font-medium uppercase tracking-wider transition-all flex items-center gap-2 border ${mostrarAdormecidos ? 'bg-gray-500/20 text-gray-200 border-gray-400/50 shadow-[0_0_15px_rgba(255,255,255,0.05)]' : 'bg-black/20 text-gray-600 border-white/5 hover:border-white/10'}`}
                >
                ...
                <div className="flex items-center gap-1">
                    <select
                        ...
                        className="h-9 bg-[#0a0a0a] border border-white/10 rounded-lg px-2 text-[10px] font-medium text-gray-300 focus:outline-none focus:border-yellow-500/50 transition-colors uppercase cursor-pointer"
                    >
                    ...
                    <select
                        ...
                        className="h-9 bg-[#0a0a0a] border border-white/10 rounded-lg px-2 text-[10px] font-medium text-gray-300 focus:outline-none focus:border-yellow-500/50 transition-colors cursor-pointer"
                    >
            ...
            <div className="overflow-x-auto rounded-xl border border-white/10 bg-black/20 backdrop-blur-md">
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-white/10 bg-white/5 uppercase tracking-widest text-gray-400 font-sans font-medium text-[9px]">
                                ...
                                <tr key={irmao.id} className={`hover:bg-white/[0.02] transition-colors group ${isAdormecido ? 'opacity-50' : ''}`}>
                                    <td className="px-5 py-4 whitespace-nowrap">
                                        <div className="flex items-center gap-2">
                                            <span className={`text-[11px] font-medium group-hover:text-yellow-500 transition-colors ${isAdormecido ? 'text-red-500/60' : 'text-gray-100'}`}>
                                                {irmao.nome}
                                            </span>
                                            {irmao.tipo_pessoa === 'candidato' && (
                                                <span className="text-[8px] text-orange-400 font-medium uppercase tracking-tighter">
                                                    CANDIDATO
                                                </span>
                                            )}
                                        </div>
                                        {isAdormecido && (
                                            <div className="text-[8px] text-red-500/60 font-medium uppercase tracking-tighter mt-0.5">
                                    ...
                                    <td className="px-5 py-4 whitespace-nowrap">
                                        {(irmao.cargo && irmao.cargo !== 'ADORMECIDO') ? (
                                            <span className="text-[8px] font-medium uppercase tracking-wider text-blue-300 bg-blue-400/10 border border-blue-400/20 px-2 py-0.5 rounded-full">{irmao.cargo}</span>
                                        ) : (
                                        ...
                                        <td className="px-5 py-4 text-center font-sans text-[10px] whitespace-nowrap min-w-[140px]">
                                            {irmao.tipo_ingresso === 'transferencia' ? (
                                                <div className="flex flex-col items-center">
                                                    ...
                                                    <span className="text-[7px] text-blue-500/50 uppercase font-medium tracking-tighter mt-0.5">Transferido</span>
                                                </div>
                                            ) : (
                                                <div className="inline-flex items-center gap-1">
                                                    <span className="text-green-400 font-medium">R$ {formatarMoeda(irmao.joia_paga)}</span>
                                                    <span className="text-gray-600">/</span>
                                                    <span className={irmao.joia_pendente > 0 ? 'text-yellow-500 font-medium' : 'text-gray-500 italic'}>
                                    ...
                                    {(categoriaFiltro === 'todas' || categoriaFiltro === 'mensalidade') && (
                                        <td className="px-5 py-4 text-center font-sans text-[10px] whitespace-nowrap min-w-[140px]">
                                            <div className="inline-flex items-center gap-1">
                                                <span className="text-green-400 font-medium">R$ {formatarMoeda(irmao.mensalidade_paga)}</span>
                                                <span className="text-gray-600">/</span>
                                                <span className={
                                                    irmao.saude_financeira === 'ATRASADO' ? 'text-red-400 font-medium' :
                                                        irmao.saude_financeira === 'PENDENTE' ? 'text-yellow-500 font-medium' :
                                    ...
                                    <td className="px-5 py-4 text-center whitespace-nowrap">
                                        <button
                                            ...
                                            className="px-3 py-1 bg-blue-600/20 text-blue-400 border border-blue-500/30 rounded text-[9px] font-medium uppercase tracking-widest hover:bg-blue-600/30 transition-all shadow-sm"
                                        >
                                            DETALHES
                                        </button>
                                    </td>
                                    <td className="px-5 py-4 text-right whitespace-nowrap">
                                        {isAdormecido ? (
                                            <div className="inline-flex items-center gap-1 text-[8px] font-medium uppercase text-gray-500 bg-gray-500/10 px-2 py-0.5 rounded-full border border-gray-500/20">Adormecido</div>
                                        ) : irmao.saude_financeira === 'REGULAR' ? (
                                            <div className="inline-flex items-center gap-1 text-[8px] font-medium uppercase text-green-400 bg-green-400/10 px-2 py-0.5 rounded-full border border-green-400/20">Regular</div>
                                        ) : irmao.saude_financeira === 'ATRASADO' ? (
                                            <div className="inline-flex items-center gap-1 text-[8px] font-medium uppercase text-red-400 bg-red-400/10 px-2 py-0.5 rounded-full border border-red-500/20">Atrasado</div>
                                        ) : (
                                            <div className="inline-flex items-center gap-1 text-[8px] font-medium uppercase text-yellow-500 bg-yellow-500/10 px-2 py-0.5 rounded-full border border-yellow-500/20">Pendente</div>
                                        )}
                                    </td>
                                </tr>
                            );
                    ...
                    <tfoot className="border-t border-white/10 bg-white/5 font-sans font-medium text-[10px] uppercase tracking-widest">
                        <tr>
                            <td colSpan={3} className="px-5 py-4 text-right text-gray-500">Totais:</td>
                            {(categoriaFiltro === 'todas' || categoriaFiltro === 'joia') && (
                                <td className="px-5 py-4 text-center whitespace-nowrap">
                                    <span className="text-green-400 font-medium">R$ {formatarMoeda(totais.joia_paga)}</span>
                                    <span className="mx-1 text-gray-600">/</span>
                                    <span className="text-yellow-500 font-medium">R$ {formatarMoeda(totais.joia_pendente)}</span>
                                </td>
                            )}
                            {(categoriaFiltro === 'todas' || categoriaFiltro === 'mensalidade') && (
                                <td className="px-5 py-4 text-center whitespace-nowrap">
                                    <span className="text-green-400 font-medium">R$ {formatarMoeda(totais.mensalidade_paga)}</span>
                                    <span className="mx-1 text-gray-600">/</span>
                                    <span className="text-red-400 font-medium">R$ {formatarMoeda(totais.mensalidade_pendente)}</span>
                                </td>
                            )}
                            <td colSpan={2}></td>
                        </tr>
                ...
                <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/80 backdrop-blur-md transition-all animate-in fade-in duration-300">
                    <div className="relative bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col m-4 animate-in zoom-in duration-300">
                        <div className="p-4 border-b border-white/10 flex justify-between items-center">
                            <div>
                                <h3 className="text-xs font-medium text-yellow-500 uppercase tracking-widest">Detalhamento Financeiro</h3>
                        ...
                        <div className="p-6 space-y-3 max-h-[60vh] overflow-y-auto">
                            <div className="text-[11px] text-gray-400 uppercase font-medium tracking-wider mb-4">
                                ...
                                {irmaoSelecionado.meses_devidos > 0 && (
                                    <>
                                        <span className="ml-3 text-gray-600">·</span>
                                        <span className="ml-3 text-red-400 font-medium font-sans">{irmaoSelecionado.meses_devidos} meses devidos</span>
                                    </>
                                )}
                            </div>
                            ...
                            <div className="flex flex-wrap gap-4 mb-6 p-4 bg-white/5 rounded-2xl border border-white/5">
                                <label className="flex items-center gap-3 cursor-pointer group">
                                    ...
                                    <span className="text-[10px] text-gray-300 font-medium uppercase tracking-widest group-hover:text-white transition-colors">Joia Paga Externamente (<span className="font-sans">Forçar R$ 2.000</span>)</span>
                                </label>
                                ...
                                <label className="flex items-center gap-3 cursor-pointer group border-l border-white/10 pl-4">
                                    ...
                                    <span className="text-[10px] text-gray-300 font-medium uppercase tracking-widest group-hover:text-white transition-colors">Isentar Mês de Iniciação (Sem dívida)</span>
                                </label>
                            </div>
                                ...
                                return (
                                    <div key={mes.mes_ref} className="space-y-2">
                                        <button
                                            ...
                                            className={`w-full flex items-center justify-between px-4 py-2.5 rounded-xl border font-medium text-[11px] uppercase tracking-wider transition-all ${
                                        ...
                                        {isPendingInput && !mes.excecao_id && (
                                            <div className="flex gap-2 pl-2">
                                                ...
                                                <button
                                                    ...
                                                    className="px-3 py-2 bg-yellow-500/20 border border-yellow-500/30 hover:bg-yellow-500/30 text-yellow-400 rounded-lg text-[10px] font-medium uppercase tracking-wider transition-all"
                                                >
                                                    Salvar
                                                </button>
                                                <button
                                                    ...
                                                    className="px-3 py-2 bg-white/5 hover:bg-white/10 text-gray-400 rounded-lg text-[10px] font-medium uppercase tracking-wider transition-all"
                                                >
                                                    Cancelar
                                                </button>
                                            </div>
                        ...
                        <div className="p-4 bg-black/20 border-t border-white/5">
                            <button
                                ...
                                className="w-full py-2.5 bg-white/5 hover:bg-white/10 text-white rounded-xl text-[10px] font-medium uppercase tracking-widest transition-all"
                            >
                                Fechar Detalhes
                            </button>
                        </div>
